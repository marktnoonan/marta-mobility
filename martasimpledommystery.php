...
if (isset($_POST['providedUsername']) && isset($_POST['providedPassword']) && $_POST['providedUsername'] !== '' && $_POST['providedPassword'] !== '' && $_POST['providedUsername'] !== 'test' && $_POST['providedPassword'] !== 'test') {
...
elseif ($_POST['providedUsername'] === 'test' && $_POST['providedPassword'] === 'test') { // fetch dummy data for test user Joanna M Customer
...
//  exit("username or password is missing!");
  ...
  //  echo json_encode($arrayOfBookings);
  ...
  // nuggets are what we pull out of the DOM, all need a little different parsing.
    ...
    elseif (strpos($nugget, 'nd Window')) {
      ...
      $arrayOfBookings[$i]['endWindow'] = formatTime(substr($nugget, 12));
    }

    elseif (strpos($nugget, 'Booked')) {

        // if found, this is last in the set we are looking for, so we increment $i here.
...
$cancelledTrip = false;
  ...
  if ($html->find('span[class=Cancelled]', 0)->plaintext !== "") {
    $cancelledTrip = true;
  }
...
$firstPickupLong = substr($latLongForFirstPickup[2], 2);
  ...
  $json = [
      (object) ['clientName' => strip_tags($customerInfo->plaintext), 'bookings' => $arrayOfBookings, 'Pick Up location data' => $latLongForFirstPickup, 'Pickup Long' => $firstPickupLong, 'updatedAt' => date('g:i A'), "Past trip found" => $pastTripInDom, "cancellationFound" => $cancelledTrip]
  ];
